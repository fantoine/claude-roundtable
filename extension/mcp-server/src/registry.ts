...
function parseRepoUrl(input: string): { owner: string; repo: string; ref: string } | null {
...
function isAgentCandidate(path: string): boolean {
  ...
  if (path.startsWith('docs/') || path.startsWith('.github/') || path.startsWith('test')) return false;
...
function parseAgentContent(content: string): { frontmatter: AgentFrontmatter; systemPrompt: string } | null {
...
async function fetchJsonFile(owner: string, repo: string, ref: string, path: string): Promise<unknown | null> {
  try {
...
async function findDeclaredAgentPaths(
  tree: GitHubTreeEntry[],
  owner: string,
  repo: string,
  ref: string,
  ...
  if (hasPluginJson) {
    const data = (await fetchJsonFile(owner, repo, ref, '.claude-plugin/plugin.json')) as PluginJson | null;
    if (data) {
      ...
      if (paths.length === 0) {
        if (tree.some((e) => e.path.startsWith('commands/'))) paths.push('commands');
        if (tree.some((e) => e.path.startsWith('agents/'))) paths.push('agents');
        if (tree.some((e) => e.path.startsWith('skills/'))) paths.push('skills');
      }
    }
  }
  ...
  if (hasMarketplace) {
    const data = (await fetchJsonFile(owner, repo, ref, '.claude-plugin/marketplace.json')) as MarketplaceJson | null;
    if (data?.plugins) {
  ...
  if (hasNestedPlugin) {
    const data = (await fetchJsonFile(owner, repo, ref, 'plugin/.claude-plugin/plugin.json')) as PluginJson | null;
    if (data) {
      ...
      if (paths.filter((p) => p.startsWith('plugin/')).length === 0) {
        if (tree.some((e) => e.path.startsWith('plugin/commands/'))) paths.push('plugin/commands');
        if (tree.some((e) => e.path.startsWith('plugin/agents/'))) paths.push('plugin/agents');
      }
    }
  }
...
export async function browseRepo(input: string): Promise<{
  agents: Array<{ path: string; name: string; description: string }>;
  repo: string;
  error?: string;
}> {
  ...
  try {
    ...
    tree = (await response.json()) as GitHubTreeResponse;
  } catch (err) {
  ...
  if (declaredPaths.length > 0) {
    ...
    candidates = tree.tree.filter((entry) => {
      ...
      if (['readme.md', 'changelog.md', 'contributing.md', 'license.md'].includes(filename)) return false;
  ...
  const agents: Array<{ path: string; name: string; description: string }> = [];
...
export async function fetchRemoteAgent(repoInput: string, agentPath: string): Promise<RemoteAgent | null> {
...
export function buildRemoteConversionPrompt(agent: RemoteAgent): string {
