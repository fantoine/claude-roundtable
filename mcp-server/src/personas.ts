...
function parsePersonaFile(content: string): ParsedPersona | null {
  const frontmatterMatch = content.match(/^---\n([\s\S]*?)\n---\n([\s\S]*)$/);
  if (!frontmatterMatch) {
    return null;
  }

  const metadata = parseYaml(frontmatterMatch[1]) as PersonaMetadata;
...
export async function mergePersonaIntoManifest(
  manifestPath: string,
  entry: ManifestEntry,
): Promise<void> {
...
export async function rebuildManifest(
  ...
  const entries: ManifestEntry[] = personas.map((p) => ({
    id: p.id,
...
function manifestEntryToPersona(entry: ManifestEntry): Persona {
...
export async function loadPersonaBodies(personas: Persona[]): Promise<Persona[]> {
  return Promise.all(
...
export async function loadAllPersonas(projectDir?: string): Promise<Persona[]> {
  ...
  let projectManifest: Manifest | null = null;
  if (projectDir) {
...
export function filterByExpertise(
  personas: Persona[],
  topic: string,
): Persona[] {
  ...
  const scored = personas.map((persona) => {
    const expertiseLower = persona.metadata.expertise.map((e) =>
      e.toLowerCase(),
    );
...
export function buildRoundtablePrompt(personas: Persona[]): string {
