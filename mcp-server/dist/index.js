...
import { loadAllPersonas, loadPersonaBodies, loadPersonasFromDir, filterByExpertise, buildRoundtablePrompt, rebuildManifest, } from './personas.js';
import { discoverAgents, buildConversionPrompt, personaOutputPath, } from './convert.js';
import { browseRepo, fetchRemoteAgent, buildRemoteConversionPrompt, } from './registry.js';
...
// --- Prompts ---
server.prompt('start-roundtable', 'Start a multi-persona roundtable discussion with all available personas', { topic: z.string().optional().describe('Optional topic to filter personas by expertise') }, async ({ topic }) => {
...
server.tool('list_sources', 'List curated repositories of agents that can be imported as roundtable personas. Use browse_agents to explore a specific source.', {}, async () => {
    try {
        ...
        const lines = data.sources.map((s) => `- **${s.repo}** — ${s.description}\n  categories: ${s.categories.join(', ')}`);
        return {
            content: [
                {
                    type: 'text',
...
server.tool('list_personas', 'List all available roundtable personas with their metadata', {}, async () => {
    const personas = await loadAllPersonas(process.cwd());
    ...
    const lines = personas.map((p) => `${p.metadata.icon} ${p.metadata.name} — ${p.metadata.title} [${p.level}]\n   source: ${p.metadata.source} | expertise: ${p.metadata.expertise.join(', ')}`);
    return {
        content: [
            {
                type: 'text',
...
server.tool('discover_agents', 'Scan for Claude agents that can be converted to personas', {}, async () => {
...
server.tool('get_conversion_prompt', 'Generate the conversion prompt for a specific Claude agent. The LLM should then execute this prompt to produce the persona file.', { agent_id: z.string().describe('Agent identifier (filename without .md)') }, async ({ agent_id }) => {
    const agents = await discoverAgents(process.cwd());
...
server.tool('browse_agents', 'Browse a GitHub repository to find Claude agents that can be imported as personas. Only use when the user explicitly asks to explore or import from a GitHub repo.', { repo: z.string().describe('GitHub repository (e.g. "owner/repo" or full URL)') }, async ({ repo }) => {
    ...
    const lines = result.agents.map((a) => `- **${a.name}** — ${a.description || '(no description)'}\n  path: \`${a.path}\``);
    return {
        content: [
            {
                type: 'text',
...
server.tool('import_persona', 'Fetch a Claude agent from a GitHub repo and generate the conversion prompt to create a persona directly. Only use when the user explicitly asks to import a specific agent from a GitHub repo.', {
...
server.tool('save_persona', 'Save a generated persona markdown file to disk. Use this after generating persona content with get_conversion_prompt or import_persona.', {
...
server.tool('refresh_manifest', 'Rescan all persona files and rebuild the manifest from scratch', {}, async () => {
...
// --- Resources ---
server.resource('persona-list', 'roundtable://personas', { mimeType: 'application/json', description: 'List of all available personas with metadata' }, async () => {
    const personas = await loadAllPersonas(process.cwd());
